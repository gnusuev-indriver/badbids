...
def get_switchback_results(df, alpha, metric_list=METRIC_LIST, groups={"control":"Control", "treatment":"GroupA"}):
    res_list = []
    for i in metric_list:
        try:
            ...
            logging.warning(f"Ошибка при обработке метрики {i[0]}: {str(e)}")
            # Создаем пустую запись для ошибочной метрики
            error_result = {
                "metric": i[0],
                "control_value": None,
...
def calculate_absolute_metrics(df_recprice, df_order_with_recprice, df_bid, group_cols):
    dfm = (#metric_calcprices_count(df_recprice, group_cols)
           metric_orders_count(df_order_with_recprice, group_cols)
